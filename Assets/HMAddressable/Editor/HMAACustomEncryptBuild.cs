...
namespace HM.Editor.HMAddressable.Editor
{
    ...
    [CreateAssetMenu(fileName = "HMAAEncrypt.asset", menuName = "HMAddressables/创建HMAA加密打包")]
    public class HMAACustomEncryptBuild:BuildScriptBase
    {
        ...
        protected override TResult BuildDataImplementation<TResult>(AddressablesDataBuilderInput builderInput)
        {
            ...
            if (result != null && string.IsNullOrEmpty(result.Error))
            {
                foreach (var group in m_IncludedGroupsInBuild)
                {
                   ...
                   //通过反射调用
                   MyEditorTools.CallStaticPrivateMethod(typeof(ContentUpdateScript),"ClearContentUpdateNotifications",group);
                }
        ...
        protected virtual TResult DoBuild<TResult>(AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext) where TResult : IDataBuilderResult
        {
            ...
            if (extractData.BuildCache != null && builderInput.PreviousContentState == null)
            {
                using (Log.ScopedStep(LogLevel.Info, "Generate Content Update State"))
                {
                    ...
                    using (Log.ScopedStep(LogLevel.Info, "Get Assets"))
                        aaContext.Settings.GetAllAssets(allEntries, false, GroupFilter);
                    ...
                    {
                        string contentStatePath = GetContentStateDataPath(false, aaContext.Settings);
                        ...
                        try
                        {
                            ...
                            if (addrResult != null)
                            {
                                ...
                                //通过反射调用
                                MyEditorTools.SetPrivateProperty(typeof(AddressablesPlayerBuildResult),addrResult,"ContentStateFilePath",contentStatePath);
                            }
            ...
            if (addrResult != null)
            {
                ...
                //通过反射调用
                MyEditorTools.SetPrivateProperty(typeof(AddressablesPlayerBuildResult),addrResult,"IsUpdateContentBuild",builderInput.PreviousContentState != null);
            ...
            List<CachedAssetState> carryOverCachedState, Dictionary<string, string> bundleToInternalId)
        {
            using (var progressTracker = new UnityEditor.Build.Pipeline.Utilities.ProgressTracker())
            {
                ...
                {
                    ContentUpdateScript.ContentUpdateContext contentUpdateContext = new ContentUpdateScript.ContentUpdateContext()
                    {
            ...
            AddressableAssetsBuildContext aaContext, Dictionary<string, string> bundleRenameMap, List<Action> postCatalogUpdateCallbacks)
        {
            ...
            List<string> outputBundleNames = new List<string>(builtBundleNames.Count);
            ...
            for (int i = 0; i < builtBundleNames.Count; ++i)
            {
                ...
                //var map = aaContext.PrimaryKeyToLocation;
                //通过反射调用
                var map=  MyEditorTools.GetPrivateProterty<Dictionary<string, ContentCatalogDataEntry>>(typeof(AddressableAssetsBuildContext),aaContext,"PrimaryKeyToLocation");
                
                if (map.TryGetValue(builtBundleNames[i], out ContentCatalogDataEntry dataEntry))
                {
        ...
        private void SetPrimaryKey(ContentCatalogDataEntry forLocation, string newPrimaryKey, AddressableAssetsBuildContext aaContext)
        {
            ...
            //通过反射调用
            var prikey = MyEditorTools.GetPrivateProterty<Dictionary<string, ContentCatalogDataEntry>>(typeof(AddressableAssetsBuildContext), aaContext,
                "PrimaryKeyToLocation");
            prikey.Remove(originalKey);
            prikey.Add(newPrimaryKey, forLocation);


            var primaryKeyToDependerLocations =
                MyEditorTools.GetPrivateProterty<Dictionary<string, List<ContentCatalogDataEntry>>>(
                    typeof(AddressableAssetsBuildContext), aaContext, "PrimaryKeyToDependerLocations");
            
            if (!primaryKeyToDependerLocations.TryGetValue(originalKey, out var dependers))
            ...
            primaryKeyToDependerLocations.Remove(originalKey);
            primaryKeyToDependerLocations.Add(newPrimaryKey, dependers);
        }
         ...
         internal static string GetContentStateDataPath(bool browse, AddressableAssetSettings settings)
        {
            ...
            {
               // var pathTmp = settings.GetContentStateBuildPath();
                
               //通过反射调用
               var pathTmp = MyEditorTools.CallPrivateMethodWithReturn<string>(typeof(AddressableAssetSettings),settings,"GetContentStateBuildPath");
